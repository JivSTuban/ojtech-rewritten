    ...
    return (
      <div className="space-y-4 sm:space-y-6 max-w-2xl mx-auto">
        ...
        <div className="bg-gray-900/60 rounded-xl p-3 sm:p-4 backdrop-blur-sm border border-gray-800/50 shadow-xl shadow-black/5">
          <div className="space-y-2.5 sm:space-y-3">
            <div className="flex justify-between items-center mb-1.5 sm:mb-2">
              <label htmlFor="github-url" className="block text-xs font-medium text-gray-300">
                Project URL
              </label>
            </div>
                <div className="relative flex-1">
                  <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    <svg className="w-4 h-4 text-gray-500/60" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <input
                    ...
                    className={`w-full pl-9 bg-black/80 border ${urlError ? 'border-red-500' : 'border-gray-700'} text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 ${urlError ? 'focus:ring-red-500/50' : 'focus:ring-gray-500/50'} focus:border-transparent transition-all duration-300`}
                  />
                </div>
            
            {/* Brief Description Input */}
            <div>
              <label htmlFor="project-description" className="block text-xs font-medium text-gray-300 mb-1">
                Brief Description
              </label>
              <textarea
                ...
                rows={2}
                className="w-full bg-black/80 border border-gray-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-500/50 focus:border-transparent transition-all duration-300"
              />
            ...
            <div>
              <label htmlFor="project-tech" className="block text-xs font-medium text-gray-300 mb-1">
                Technologies Used
              </label>
              <input
                type="text"
            ...
            <div className="flex justify-end mt-2.5 sm:mt-3">
                <button
                  onClick={this.handleAddProject}
                disabled={isValidating || isEnhancing}
                  className={`
                    px-4 py-2 rounded-lg font-medium text-white flex items-center text-sm
              ...
              {error && (
                <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-red-400 mt-2">
        ...
        {projects.length > 0 ? (
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
              {projects.map((project, index) => (
                <div 
                  key={index} 
                  className="group bg-gradient-to-br from-gray-900/80 to-black/90 rounded-xl p-3 sm:p-4 md:p-5 border border-gray-800/50 flex flex-col hover:shadow-lg hover:shadow-black/20 hover:border-gray-700/50 transition-all duration-300"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                      <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-gray-800/50 rounded-lg flex items-center justify-center">
                        <svg className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                        </svg>
                      </div>
                      <h5 className="font-bold text-white text-sm sm:text-base md:text-lg truncate">{project.name}</h5>
                    </div>
                    <button
                      onClick={() => this.handleRemoveProject(index)}
                      className="text-gray-500/60 hover:text-red-400 transition-colors p-1 opacity-70 group-hover:opacity-100 flex-shrink-0 ml-2"
                  ...
                  {/* Project Description */}
                  {project.description && (
                    <p className="text-xs sm:text-sm text-gray-400 mt-2 sm:mt-3 line-clamp-3">
                      {project.description}
                    </p>
                  )}
                  ...
                  <a 
                    ...
                    className="text-xs sm:text-sm text-gray-400 hover:text-gray-300 mt-2 sm:mt-3 hover:underline truncate"
                  >
                    {project.url}
                  </a>
                  ...
                  {(project.stars !== undefined || project.lastUpdated) && (
                    <div className="flex gap-2 sm:gap-3 mt-2 sm:mt-3">
                      {project.stars !== undefined && (
                        <span className="text-[10px] sm:text-xs flex items-center gap-1 text-gray-400">
                  ...
                  {project.technologies && project.technologies.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2 sm:mt-3 md:mt-4">
                      {project.technologies.slice(0, 5).map((tech, techIndex) => (
                        <span 
                          key={techIndex}
                          className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-gray-800/50 text-gray-300 border border-gray-700/50"
                        >
                          {tech}
                        </span>
                      ))}
                      {project.technologies.length > 5 && (
                        <span className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full bg-gray-800/50 text-gray-300 border border-gray-700/50">
          ...
          <div className="bg-gray-900/30 border border-gray-800/30 rounded-xl p-4 sm:p-6 shadow-inner">
            <div className="flex flex-col items-center text-center">
              <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gray-900/50 flex items-center justify-center mb-3 sm:mb-4">
                <svg className="w-6 h-6 sm:w-8 sm:h-8 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" />
                </svg>
              </div>
              <h5 className="text-base sm:text-lg font-medium text-white mb-1 sm:mb-2">No projects added yet</h5>
        ...
        <div className="flex justify-end pt-3 sm:pt-4">
          <button
            ...
            className="px-4 py-2 rounded-lg font-medium text-white bg-gradient-to-r from-gray-600 to-gray-800 hover:from-gray-700 hover:to-gray-900 hover:shadow-lg hover:shadow-black/20 transition-all duration-300 flex items-center gap-1.5 text-sm"
          >
